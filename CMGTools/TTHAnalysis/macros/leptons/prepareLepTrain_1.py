...
class LepTreeProducer(Module):
    ...
    def analyze(self,event):
        for i in range(event.nLepGood):
            print event.LepGood_sip3d[i], " ", event.LepGood_pdgId[i], " ", event.LepGood_pt[i], " ", event.LepGood_eta[i], " ", event.LepGood_mcMatchId[i], " ", event.LepGood_mcMatchAny[i]
        ...
        for i in range(event.nLepGood):
            #self.t.sip3d = ROOT.scaleSip3dMC(event.LepGood_sip3d[i], event.LepGood_pdgId[i],event.LepGood_pt[i],event.LepGood_eta,event.LepGood_mcMatchId,event.LepGood_mcMatchAny) if self.corr else l.sip3d
