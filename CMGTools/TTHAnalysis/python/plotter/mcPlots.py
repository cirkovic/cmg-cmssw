                        ...
                        if ext == "txt":
                            ...
                            if "TH2" in total.ClassName() or "TProfile2D" in total.ClassName():
                                ...
                                for p in mca.listSignals(allProcs=True) + mca.listBackgrounds(allProcs=True) + ["signal", "background"]:
                                    ...
                                    if (plot.GetName() in ["MVA_2D_2LSS_TT", "MVA_2D_2LSS_WZ", "MVA_2D_2LSS_TTZ", "MVA_2D_2LSS_TTW", "MVA_2D_2LSS_signal"]):
                                        ...
                                        if nCats == 2:
                                            ...
                                            v1.append(plot.Integral(0,                                plot.GetXaxis().GetNbins()/3,   2*plot.GetYaxis().GetNbins()/3+1, plot.GetYaxis().GetNbins()+1))
                                            v2.append(plot.Integral(plot.GetXaxis().GetNbins()/3+1,   2*plot.GetXaxis().GetNbins()/3, 2*plot.GetYaxis().GetNbins()/3+1, plot.GetYaxis().GetNbins()+1))
                                            v3.append(plot.Integral(2*plot.GetXaxis().GetNbins()/3+1, plot.GetXaxis().GetNbins()+1,   2*plot.GetYaxis().GetNbins()/3+1, plot.GetYaxis().GetNbins()+1))
                                            v4.append(plot.Integral(0,                                plot.GetXaxis().GetNbins()/3,   plot.GetYaxis().GetNbins()/3+1,   2*plot.GetYaxis().GetNbins()/3))
                                            v5.append(plot.Integral(plot.GetXaxis().GetNbins()/3+1,   2*plot.GetXaxis().GetNbins()/3, plot.GetYaxis().GetNbins()/3+1,   2*plot.GetYaxis().GetNbins()/3))
                                            v6.append(plot.Integral(2*plot.GetXaxis().GetNbins()/3+1, plot.GetXaxis().GetNbins()+1,   plot.GetYaxis().GetNbins()/3+1,   2*plot.GetYaxis().GetNbins()/3))
                                            v7.append(plot.Integral(0,                                plot.GetXaxis().GetNbins()/3,   0,                                plot.GetYaxis().GetNbins()/3))
                                            v8.append(plot.Integral(plot.GetXaxis().GetNbins()/3+1,   2*plot.GetXaxis().GetNbins()/3, 0,                                plot.GetYaxis().GetNbins()/3))
                                            v9.append(plot.Integral(2*plot.GetXaxis().GetNbins()/3+1, plot.GetXaxis().GetNbins()+1,   0,                                plot.GetYaxis().GetNbins()/3))
                                ...
                                if nCats >= 2:
                                    ...
                                    pie1.Draw("nol <")
                                    c21.Print("%s/%s.%s" % (fdir, "ThePie_1", ext))

                                    c22 = ROOT.TCanvas()
                                    pie2 = ROOT.TPie("ThePie", "ThePie", len(v2), v2, c, a)
                                    pie2.SetRadius(.2)
                                    pie2.SetLabelsOffset(0.01)
                                    pie2.SetLabelFormat("#splitline{%val (%perc)}{%txt}")
                                    pie2.Draw("nol <")
                                    c22.Print("%s/%s.%s" % (fdir, "ThePie_2", ext))

                                    c23 = ROOT.TCanvas()
                                    pie3 = ROOT.TPie("ThePie", "ThePie", len(v3), v3, c, a)
                                    pie3.SetRadius(.2)
                                    pie3.SetLabelsOffset(0.01)
                                    pie3.SetLabelFormat("#splitline{%val (%perc)}{%txt}")
                                    pie3.Draw("nol <")
                                    c23.Print("%s/%s.%s" % (fdir, "ThePie_3", ext))

                                    c24 = ROOT.TCanvas()
                                    pie4 = ROOT.TPie("ThePie", "ThePie", len(v4), v4, c, a)
                                    pie4.SetRadius(.2)
                                ...
                                if nCats >= 3:
                                    c25 = ROOT.TCanvas()
                                    pie5 = ROOT.TPie("ThePie", "ThePie", len(v5), v5, c, a)
                                    pie5.SetRadius(.2)
                                    pie5.SetLabelsOffset(0.01)
                                    pie5.SetLabelFormat("#splitline{%val (%perc)}{%txt}")
                                    pie5.Draw("nol <")
                                    c25.Print("%s/%s.%s" % (fdir, "ThePie_5", ext))

                                    c26 = ROOT.TCanvas()
                                    pie6 = ROOT.TPie("ThePie", "ThePie", len(v6), v6, c, a)
                                    pie6.SetRadius(.2)
                                    pie6.SetLabelsOffset(0.01)
                                    pie6.SetLabelFormat("#splitline{%val (%perc)}{%txt}")
                                    pie6.Draw("nol <")
                                    c26.Print("%s/%s.%s" % (fdir, "ThePie_6", ext))

                                    c27 = ROOT.TCanvas()
                                    pie7 = ROOT.TPie("ThePie", "ThePie", len(v7), v7, c, a)
                                    pie7.SetRadius(.2)
                                    pie7.SetLabelsOffset(0.01)
                                    pie7.SetLabelFormat("#splitline{%val (%perc)}{%txt}")
                                    pie7.Draw("nol <")
                                    c27.Print("%s/%s.%s" % (fdir, "ThePie_7", ext))

                                    c28 = ROOT.TCanvas()
                                    pie8 = ROOT.TPie("ThePie", "ThePie", len(v8), v8, c, a)
                                    pie8.SetRadius(.2)
                                    pie8.SetLabelsOffset(0.01)
                                    pie8.SetLabelFormat("#splitline{%val (%perc)}{%txt}")
                                    pie8.Draw("nol <")
                                    c28.Print("%s/%s.%s" % (fdir, "ThePie_8", ext))

                                    c29 = ROOT.TCanvas()
                                    pie9 = ROOT.TPie("ThePie", "ThePie", len(v9), v9, c, a)
                                    pie9.SetRadius(.2)
                                    pie9.SetLabelsOffset(0.01)
                                    pie9.SetLabelFormat("#splitline{%val (%perc)}{%txt}")
                                    pie9.Draw("nol <")
