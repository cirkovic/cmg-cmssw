...
def scalarToVector(x):
    x0 = x
    x = re.sub(r"(LepOther|LepGood|Lep|JetFwd|Jet|GenTop|SV)(\d)_(\w+)", lambda m : "%s_%s[%d]" % (m.group(1),m.group(3),int(m.group(2))-1), x)
...
#                  self._printArrEl(b_met_pt, form='10.1f')
                  self._printArrEl(b_met_genPt, form='10.1f')
#                  self._printArrEl(b_met_phi, form='+7.2f')
                  self._printArrEl(b_met_genPhi, form='+7.2f')
