...
#            print cut
            ...
            if self._options.prtVars and self._file_i > 0:
               ri = randint(0, sys.maxint)
               f_dummy1 = TFile.Open('f_dummy1'+str(ri)+'.root','RECREATE')
               ...
               f_dummy2 = TFile.Open('f_dummy2'+str(ri)+'.root','RECREATE')
               tree2 = tree1.CopyTree("")
               f_dummy2.Write()
               #self._o_txt = open(self._options.outDir+'/cut'+str(self._file_i)+'.txt', 'wb')
               #self._o_txt = open(self._options.outDir+'/cut'+str(self._file_i)+'.txt', 'a')
               self._o_txt = open(self._options.outDir+'/cut'+str(self._file_i)+'.txt', 'w')
               self._txt_delimiter = ' '
               self._csv_delimiter = ','
               if self._options.lepTyp == "muon":
                  #self._o_txt.write("     event      pT  Eta  Phi      E pdgID charge    miniIso miniIsoCharged miniIsoNeutral   jetPtRel jetCSV jetPtRatio  sip3D    dxy     dz  segmentCompatibility\n")
                  self._o_txt.write("     event        pT       Eta       Phi         EpdgIDcharge        miniIso miniIsoCharged miniIsoNeutral  jetPtRel    jetCSVjetPtRatio     sip3D       dxy        dz segmentCompatibility\n")
                  for ev in tree2:
                  #if (ev.nLepGood > 0):
                      #for li in xrange(0, ev.nLepGood):
                      #    if ev.LepGood_pdgId[li] == 13:
                      #        break
                      li = 0
                      p4 = ROOT.TLorentzVector()
                      p4.SetPtEtaPhiM(ev.LepGood_pt[0], ev.LepGood_eta[0], ev.LepGood_phi[0], ev.LepGood_mass[0])
                      self._o_txt.write(
                      #"%10d  %6.2f %+4.2f %+4.2f %6.2f %+2d %+2d    %6.1f %6.1f %6.1f   %6.1f %6.1f %6.1f %6.1f %6.1f %6.1f  %6.1f  \n" % (
                      "%10d%10.5f%10.5f%10.5f%10.5f%5d%5d%15.5f%15.5f%15.5f%10.5f%10.5f%10.5f%10.5f%10.5f%10.5f%21.5f\n" % (
                          ev.evt,
                          ev.LepGood_pt[li], ev.LepGood_eta[li], ev.LepGood_phi[li], p4.E(), ev.LepGood_pdgId[li], ev.LepGood_charge[li],
                          ev.LepGood_miniRelIso[li],
                          ev.LepGood_miniRelIsoCharged[li],
                          ev.LepGood_miniRelIsoNeutral[li],
                          ev.LepGood_jetPtRelv2[li],
                          ev.LepGood_jetBTagCSV[li],
                          ev.LepGood_jetPtRatiov2[li],
                          ev.LepGood_sip3d[li],
                          ev.LepGood_dxy[li],
                          ev.LepGood_dz[li],
                          ev.LepGood_segmentCompatibility[li]
                      ))
               elif self._options.lepTyp == "electron":
                  #self._o_txt.write("     event      pT  Eta  Phi      E pdgID charge    miniIso miniIsoCharged miniIsoNeutral   jetPtRel jetCSV jetPtRatio  sip3D    dxy     dz  eleMVA\n")
                  self._o_txt.write("     event        pT       Eta       Phi         EpdgIDcharge        miniIso miniIsoCharged miniIsoNeutral  jetPtRel    jetCSVjetPtRatio     sip3D       dxy        dz               eleMVA\n")
                  for ev in tree2:
                      #for li in xrange(0, ev.nLepGood):
                      #    if ev.LepGood_pdgId[li] == 11:
                      #        break
                      li = 0
                      p4 = ROOT.TLorentzVector()
                      p4.SetPtEtaPhiM(ev.LepGood_pt[0], ev.LepGood_eta[0], ev.LepGood_phi[0], ev.LepGood_mass[0])
                      ...
                      "%10d%10.5f%10.5f%10.5f%10.5f%5d%5d%15.5f%15.5f%15.5f%10.5f%10.5f%10.5f%10.5f%10.5f%10.5f%21.5f\n" % (
                          ...
                          ev.LepGood_dxy[li],
                          ev.LepGood_dz[li],
                          #ev.LepGood_mvaIdPhys14[li],
                          ev.LepGood_mvaIdSpring15[li],
                      ))
                  '''
                  ...
                  if self._file_i == 0 and self._options.extPrt:
                      ...
                      passing_cutc = 1 if ev.LepGood_tightId[0] > (abs(ev.LepGood_pdgId[0]) == 11) and ev.LepGood_tightId[1] > (abs(ev.LepGood_pdgId[
                      passing_cutd = 1 if max(ev.LepGood_sip3d[0],ev.LepGood_sip3d[1]) < 4 else 0
                      passing_cute = 1 if (abs(ev.LepGood_pdgId[0]) == 13 or (ev.LepGood_convVeto[0] and ev.LepGood_lostHits[0] == 0 and ev.LepGood_t
               ...
               os.system('rm f_dummy1'+str(ri)+'.root f_dummy2'+str(ri)+'.root')
...
def addTreeToYieldOptions(parser):
    ...
    parser.add_option("--lepType", "--lepton-type", dest="lepTyp",  type="string", default=None, help="Lepton type (\"electron\" or \"muon\")")
